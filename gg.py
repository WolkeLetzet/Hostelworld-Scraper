...
class GUI :
   ...
   def __init__(self,path,*parent:Tk) -> None:
      ...
      #### VARIABLES ####
      self.options = [tk.BooleanVar(),tk.BooleanVar(),tk.BooleanVar()] #variables de opciones
   ...
   def build_cBoxes(self):
      ...
      self.continentsLabel = tk.Label(self.cBoxes_frame,text="Continente:", font=(FONT, FSIZE2), fg="white",background=ORANGE)
      self.countriesLabel = tk.Label(self.cBoxes_frame,text="Pais:", font=(FONT, FSIZE2), fg="white",background=ORANGE)
      self.citiesLabel = tk.Label(self.cBoxes_frame,text="Ciudad:", font=(FONT, FSIZE2), fg="white",background=ORANGE)
      
      self.continentsBox  = ttk.Combobox(self.cBoxes_frame,state='readonly',font=(FONT, FSIZE3),values=tuple(self.continentes.keys()))
      self.countriesBox  = ttk.Combobox(self.cBoxes_frame,state='readonly',font=(FONT, FSIZE3))
      self.citiesBox  = ttk.Combobox(self.cBoxes_frame,state='readonly',font=(FONT, FSIZE3))
   ...
   def build_progressBar_window(self,max):
      ...
      self.progbar = ttk.Progressbar(self.pgw,maximum=max,variable=self.progVar,length=200)
